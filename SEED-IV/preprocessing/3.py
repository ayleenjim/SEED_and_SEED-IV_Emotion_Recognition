...
for trial in trials:
    # Ensure that the number of samples is divisible by the window size
    n_samples = trial.shape[1]
    n_windows = n_samples // window_size  # Integer division to get full windows
    trimmed_samples = n_windows * window_size  # Calculate the exact number of samples used
    trial = trial[:, :trimmed_samples]  # Trim the trial data if necessary
...
# Convert the list of trials into a numpy array (all trials should now have the same shape)
