...
for trial in trials:
    ...
    psd_trial = np.zeros((n_channels, (n_samples // nperseg), len(freq_bands)))

    # Compute PSD for each channel using Welch's method
    for ch in range(n_channels):
        # Welch's method computes PSD for each 4-second window
        freqs, psd = signal.welch(trial[ch], fs=sampling_rate, nperseg=nperseg, noverlap=noverlap, nfft=nperseg)
